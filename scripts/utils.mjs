...
export function getAllProgramIdls() {
  return getAllProgramFolders().map((folder) =>
    path.join(workingDirectory, 'apps', 'program', folder, 'idl.json')
  );
}

export function getExternalProgramOutputDir() {
  const config = getCargoMetadata()?.solana?.['external-programs-output'];
  return path.join(workingDirectory, 'apps/program', config ?? 'target/deploy');
}
  ...
  if (
    ...
    programs
      ...
      .forEach((program) => {
        echo(
          chalk.yellow(
            `Program not found: ${workingDirectory}/apps/program/${program}`
          )
        );
      });
  }
